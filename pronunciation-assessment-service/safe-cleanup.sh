...
# Safe cleanup script for pronunciation-assessment-service
...
print_status "=========================================="
print_status "Safe Cleanup: Pronunciation Assessment Service"
print_status "=========================================="

# Warning
if [ "$FORCE" = false ]; then
    print_warning "⚠️  This will remove pronunciation-assessment-service containers and images"
...
print_status "Removing pronunciation-assessment containers by name..."
CONTAINERS=$(docker ps -a --filter "name=pronunciation-assessment" --format "{{.Names}}" | grep -E '^pronunciation-assessment' || true)
if [ -n "$CONTAINERS" ]; then
    ...
    print_status "No pronunciation-assessment containers found"
fi

# Remove specific images
print_status "Removing pronunciation-assessment-service images..."
IMAGES=$(docker images --filter "reference=pronunciation-assessment-service*" --format "{{.ID}}" || true)
if [ -n "$IMAGES" ]; then
    ...
    print_status "No pronunciation-assessment-service images found"
fi

# Clean up orphaned volumes specific to this project
print_status "Cleaning up project-specific volumes..."
VOLUMES=$(docker volume ls --filter "name=pronunciation" --format "{{.Name}}" || true)
if [ -n "$VOLUMES" ]; then
    ...
    print_status "No pronunciation-related volumes found"
fi
...
print_success "=========================================="
print_status "Removed: pronunciation-assessment-service containers and images"
